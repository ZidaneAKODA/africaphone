...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Fonction qui applique les attributs
  ...
  // 1) On traite tous les inputs déjà présents
  document.querySelectorAll('input').forEach(disableInputs);

  // 2) On observe les ajouts dans le DOM (pour les inputs créés dynamiquement)
  const observer = new MutationObserver(mutations => {
    for (const m of mutations) {
      for (const node of m.addedNodes) {
        if (node.nodeType !== 1) continue;             // on ne s’intéresse qu’aux éléments
        if (node.tagName === 'INPUT') {
          disableInputs(node);
        }
        // et à tout input qu’il contiendrait
        node.querySelectorAll && node.querySelectorAll('input')
            .forEach(disableInputs);
      }
    }
  });
